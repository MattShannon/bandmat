...
def plus_equals_band_of(target_bm, mat_bm):
    """Adds one matrix to another in-place.

    Any entries of `mat_bm` which lie outside of `target_bm` are ignored.
    Thus to implement conventional matrix addition, `target_bm` must be large
    enough to contain the result of the addition, i.e. `target_bm` must have at
    least as many subdiagonals and superdiagonals as `mat_bm`.

    The statement `plus_equals_band_of(target_bm, mat_bm)` where `target_bm`
    ...
    l_a = target_bm.l
    u_a = target_bm.u
    a_data = target_bm.data
    transposed_a = target_bm.transposed
    assert l_a >= 0
    assert u_a >= 0
...
def plus(a_bm, b_bm):
    ...
    The expression `plus(a_bm, b_bm)` where `a_bm` and `b_bm` are BandMats is
    the equivalent of:
...
def scalar_mult(a_bm, mult, overwrite = False):
    """Multiplies a banded matrix by a scalar.

    The expression `scalar_mult(a_bm, mult)` where `a_bm` is a BandMat is the
    equivalent of:

        a_full * mult

    where `a_full` is a square numpy array.
    The statement `scalar_mult(a_bm, mult, overwrite=True)` is the equivalent
    of:
    ...
    if overwrite:
        ...
        return bm_core.BandMat(a_bm.l, a_bm.u, a_bm.data * mult,
